...
-- Lịch sử vị trí target để prediction-TP (giống auto-bounty-m1)
local _enemyHistory = {}
local PREDICT_RATIO = 65 / 140
local MAX_PREDICT_SAMPLES = 10
    ...
    if hrp then
        local ok = pcall(function()
            ...
            for _, v in pairs(zones:GetChildren()) do
                if v:IsA("Part") and (v.Position - hrp.Position).Magnitude <= 400 then
                    error("in")
                end
            end
        end)
...
local function IsValidTarget(p)
    ...
    if IsPvPDisabledAttr(p) then return false end
    -- Bỏ qua nếu đang raid
    local raidMap = workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("RaidMap")
    if raidMap and char:IsDescendantOf(raidMap) then return false end
    if not IsValidLevel(p) then return false end
    return true
end
...
-- Prediction-based teleport (theo auto-bounty-m1 style)
local function PredictTeleportTo(targetPlayer)
    pcall(function()
        ...
        if not _enemyHistory[name] then
            _enemyHistory[name] = {
                lastPos = currentPos,
            ...
            -- TP thẳng lần đầu
            myHRP.CFrame = CFrame.new(currentPos + Vector3.new(7, 5, 0), currentPos)
            return
        end

        local data = _enemyHistory[name]
        ...
        if deltaTime > 0 then
            ...
            if #data.speeds > MAX_PREDICT_SAMPLES then
                table.remove(data.speeds, 1)
            end

            local sumSpeed = 0
            for _, s in ipairs(data.speeds) do sumSpeed = sumSpeed + s end
            local avgSpeed = sumSpeed / #data.speeds
            ...
            if avgSpeed > 0.5 then
                ...
                local predictStud = avgSpeed * PREDICT_RATIO
                basePos = currentPos + (finalDir * predictStud)
            else
                basePos = currentPos + (targetHRP.CFrame.LookVector * 5)
            end

            -- Random offset để không đứng yên một chỗ (safe hơn)
            local offsets = {
                Vector3.new(7, 5, 0),
                Vector3.new(-7, 5, 0),
                Vector3.new(0, 5, 7),
            ...
            myHRP.CFrame = CFrame.new(finalTpPos, currentPos)
        end
    end)
end
...
local function AttackPlayer(targetPlayer, reason)
    ...
    if not IsValidTarget(targetPlayer) then
        SetText("Skip " .. targetPlayer.Name .. " (invalid target)")
        task.wait(0.5)
        return false
    end

    -- Bật PvP nếu chưa bật
    ...
    repeat
        ...
        if dist <= 45 then
            ...
            -- Ken mỗi 10s
            if tick() - _lastV3KenCall >= 10 then
                _lastV3KenCall = tick()
    ...
    return killed or (targetHum ~= nil and targetHum.Health <= 0)
end
...
-- ─────────────────────────────────────────────────
-- UP V3 GHOUL — Quest: Kill 5 Players (wiki rules)
-- • Kill 5 players WITHOUT dying (nếu chết → quest reset)
-- • Cùng 1 player có thể kill nhiều lần
-- • Cần kill thêm (target 7) vì glitch không register
-- • Quest reset mỗi khi join server mới → phải nhận lại
-- • Friendly PvP được phép
-- • Kill "suspicious" không tính → phải deal dmg thật
-- ─────────────────────────────────────────────────
...
local function FindValidV3Target(blacklist)
    ...
    for _, p in ipairs(players) do
        if not (blacklist and blacklist[p.Name]) then
...
local function GetV3()
    ...
    -- ── DEATH WATCHER ──────────────────────────────
    -- Nếu mình chết trong khi quest đang chạy:
    -- server sẽ reset kill count về 0 → phải bắt đầu lại
    local myDeathDetected = false
    local myDeathConn = nil

    local function BindDeathWatcher()
        if myDeathConn then pcall(function() myDeathConn:Disconnect() end) end
        ...
        myDeathConn = hum.Died:Connect(function()
            myDeathDetected = true
            SetText("[Ghoul V3] ⚠️ Đã chết! Kill count reset → restart...")
        end)
    end
    ...
    while not getgenv().StopV3 do
        ...
        if myDeathDetected then
            ...
            BindDeathWatcher()
            SetText("[Ghoul V3] Respawn xong, bắt đầu lại từ 0...")
            task.wait(2)
            continue
        end
